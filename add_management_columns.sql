...
-- 2. Migrar dados antigos para o novo padrão (CRÍTICO: fazer antes de adicionar a constraint)
...
UPDATE public.requests SET status = 'open' WHERE status IS NULL OR status NOT IN ('open', 'in_progress', 'resolved');

-- 3. Constraint de segurança para status
ALTER TABLE public.requests
...
-- 4. Índices de performance para multi-tenancy e dashboard
